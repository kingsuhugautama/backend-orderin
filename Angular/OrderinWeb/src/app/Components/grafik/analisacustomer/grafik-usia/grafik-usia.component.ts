...
@Component({
  selector: 'app-grafik-usia',
  templateUrl: './grafik-usia.component.html',
  styleUrls: ['./grafik-usia.component.css']
})
export class GrafikUsiaComponent implements OnInit {
  ...
  constructor(private custom:CustomService,
    ...
    bindDataSource(param = []){
      ...
      this.custom.setAsyncDataSource('Grafik/getAnalisaCustomerPerUsia',param)
      .subscribe((res)=>{
        console.log(res);
        let data = res.data;
        if(data.length > 0){
          ...
          for(var index in data){
            this.piedata.push({
              x:data[index]['usia'],
              y:data[index]['jumlah'],
              text:data[index]['usia'] + ' ( ' + data[index]['jumlah'].toString() + ' )',
